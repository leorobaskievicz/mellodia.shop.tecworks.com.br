...
const urlReplacements = [
  ...
  { from: "https://dricor.cdn.tecworks.com.br", to: config.CDN_URL },
...
if (fs.existsSync(apiPath)) {
  ...
  apiContent = apiContent.replace(/this\.url = ["']https:\/\/api\.divacosmeticos\.com\.br["']/g, "this.url = process.env.NEXT_PUBLIC_API_URL");

  // Substituir token hardcoded por variável de ambiente
  apiContent = apiContent.replace(/constructor\(token = ["'][^"']+["']\)/, "constructor(token = process.env.NEXT_PUBLIC_API_TOKEN)");

  // Substituir X-Auth-Token hardcoded
  apiContent = apiContent.replace(/["']X-Auth-Token["']:\s*["'][^"']+["']/g, '"X-Auth-Token": process.env.NEXT_PUBLIC_API_X_AUTH_TOKEN');
...
if (fs.existsSync(algoliaInsightsPath)) {
  ...
  content = content.replace(/appId:\s*["'][^"']+["']/, "appId: process.env.NEXT_PUBLIC_ALGOLIA_APP_ID");

  content = content.replace(/apiKey:\s*["'][^"']+["']/, "apiKey: process.env.NEXT_PUBLIC_ALGOLIA_RECOMMEND_API_KEY");
