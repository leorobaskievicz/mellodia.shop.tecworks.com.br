...
import { Box, Checkbox, Container, TextField, Button, Typography, Link, InputAdornment, IconButton, CircularProgress, Alert, FormControlLabel, Grid, Paper, Select, MenuItem } from "@mui/material";
...
function CadastroContent() {
  ...
  const handleSubmitNovo = async (event) => {
    ...
    if (state.currentStep === 1) {
      ...
      if (!state.novoIeIsento && !String(state.novoIe).trim()) {
        setMsg("error", "Inscrição Estadual", `Informe a Inscrição Estadual ou marque "Isento".`, 2);
        return;
      }
    ...
    const param = {
      ...
      nome: state.novoNome,
      apelido: state.novoApelido,
      cpf: state.novoCpf,
      ie: state.novoIeIsento ? "ISENTO" : state.novoIe,
    ...
    try {
      ...
      if (!data.status) {
        ...
        // Mesma sessão do login: sem o cookie, quem recarrega a página antes de
        // qualquer outra ação volta deslogado.
        ...
        setTimeout(() => {
          setState((state) => ({ ...state, redirect: destino && destino.startsWith("/") ? destino : "/" }));
        }, 400);
      }
    } catch (e) {
      console.error(e);
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <TextField
                    fullWidth
                    label="Nome Fantasia"
                    value={state.novoApelido}
                    onChange={(event) =>
                      setState((state) => ({
                        ...state,
                        novoApelido: event.target.value,
                      }))
                    }
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={8}>
                  <TextField
                    ...
                    value={state.novoIeIsento ? "ISENTO" : state.novoIe}
                    onChange={(event) =>
                      setState((state) => ({
                        ...state,
                        novoIe: event.target.value,
                      }))
                    }
                    disabled={state.novoIeIsento}
                    required={!state.novoIeIsento}
                  />
                </Grid>
                ...
                <Grid item xs={12} sm={4} sx={{ display: "flex", alignItems: "center" }}>
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={state.novoIeIsento}
                        onChange={(event) =>
                          setState((state) => ({
                            ...state,
                            novoIeIsento: event.target.checked,
                          }))
                        }
                      />
                    }
